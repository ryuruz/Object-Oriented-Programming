...
비교 | 목적 | 구성
:---:|:---:|:---:
추상클래스 | 서브 클래스에서 필요로 하는 대부분의 기능을 구현하여 두고, 서브 클래스에서 구현해야만 하는 것들을 추상 메소드로 선언하여 서브 클래스가 상속받아 서브클래스에서 구현하도록 함| 추상 메소드, 일반 메소드 모두 포함 &  상수, 변수 필드 모두 포함
인터페이스 | 객체의 기능을 모두 공개한 표준화 문서. 클래스의 목적에 따라 인터페이스의 추상 메소드를 만들도록 함| 변수 필드는 포함하지 않음 & 상수, 추상 메소드, 일반 메소드, default, static 메소드 모두 포함 & protected 접근 지정 선언 불가 & 다중 상속 지원
